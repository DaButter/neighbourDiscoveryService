...
uint32_t getInterfaceIPv4(const char* ifname) {
    ...
    close(temp_sock);  // Close temporary socket

    struct sockaddr_in* addr = reinterpret_cast<struct sockaddr_in*>(&ifr.ifr_addr);
    return addr->sin_addr.s_addr;
}

// get IPv6 address for interface (returns false if none)
// use getifaddrs() (no socket needed)
bool getInterfaceIPv6(const char* ifname, uint8_t ipv6[16]) {
    ...
    for (struct ifaddrs* ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        if (ifa->ifa_addr->sa_family == AF_INET6) {
            ...
            std::memcpy(ipv6, addr6->sin6_addr.s6_addr, 16);
            found = true;
            break;
        }
    }
