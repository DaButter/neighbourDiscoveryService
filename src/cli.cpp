...
int main() {
    ...
    if (connect(sockfd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        std::cerr << "Failed to connect to service: " << strerror(errno) << std::endl;
        close(sockfd);
        return 1;
    }
    ...
    if (recv(sockfd, &neighborCount, sizeof(neighborCount), MSG_WAITALL) != sizeof(neighborCount)) {
        std::cerr << "Failed to receive neighbor count: " << strerror(errno) << std::endl;
        close(sockfd);
        return 1;
    }
    ...
    for (uint32_t i = 0; i < neighborCount; ++i) {
        ...
        if (recv(sockfd, &info, sizeof(info), MSG_WAITALL) != sizeof(info)) {
            std::cerr << "Failed to receive neighbor info: " << strerror(errno) << std::endl;
            close(sockfd);
            return 1;
        }
        ...
        for (uint32_t j = 0; j < info.connectionCount; ++j) {
            ...
            if (recv(sockfd, &conn, sizeof(conn), MSG_WAITALL) != sizeof(conn)) {
                std::cerr << "Failed to receive connection info: " << strerror(errno) << std::endl;
                close(sockfd);
                return 1;
            }
